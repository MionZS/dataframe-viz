#!/usr/bin/env python3
"""
Inspetor de Dados CSV - Visualiza tipos e primeiras linhas.

Funcionalidades:
  - Lê primeiras N linhas de um CSV
...
def file_picker(directory: str) -> str:
    """Interactive file picker for CSV files."""
    ...
    while True:
        ...
        console.print(f"Mostrar arquivos CSV: {toggle_status}  (pressione 't' para alternar)\n")

        csv_files = sorted([p for p in dir_path.glob("*.csv")]) if show_files else []
        ...
        if choice.lower() == "q":
            ...
            if choice_num in dir_indices:
                ...
                return str(file_indices[choice_num])
            else:
                console.print("[red]Opção inválida[/red]")
                Prompt.ask("[yellow]Pressione Enter[/yellow]")
        else:
...
class DataInspector:
    ...
    def _load_sample(self):
        ...
        try:
            ...
            self.console.print(f"[green]✓[/green] Arquivo carregado: {self.file_path.name}")
    ...
    def run(self):
        ...
        while True:
            ...
            self.console.print(Panel(
                ...
                f"Colunas: {len(self.columns)} | Amostra: {len(self.df)} linhas",
                expand=False
            ))
...
def main():
    parser = argparse.ArgumentParser(
        description="Inspetor de Dados CSV"
    )
    parser.add_argument(
        "--file", "-f",
        type=str,
    ...
    if args.file:
        ...
        console.print("[bold cyan]Inspetor de Dados CSV - Explorer[/bold cyan]")
        start_dir = Path.cwd()
